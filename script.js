...
async function run() {
  let browser = await puppeteer.launch(
    //browser opened
    {
      ...
      defaultViewport: null, //for making the content full screen
    }
  );
  ...
  let numPages = await page.$eval("a[data-attr1='Last']", function (atag) {
    let totalPages = parseInt(atag.getAttribute("data-page")); //data-page name ka attribute read
    return totalPages;
  });
  ...
  for (let i = 1; i <= numPages; i++) {
    ...
    if (i != numPages) {
      //last page pe phonchne se phle hoga yeh kaam:(upar if condition yahi denote kr rhi h)
...
async function handleAllContestsOfAPage(page, browser) {
  ...
  for (let i = 0; i < curls.length; i++) {
    //loop in all the urls
    let curl = curls[i];
    let ctab = await browser.newPage(); //new tab open
    //main work->
    await saveModeratorInAcontest(
      ctab,
      args.url + curls[i],
      configJSO.moderators
    );
    await ctab.close(); //tab close after work done
    await ctab.waitFor(3000); //after closing the tab wait on original page
  }
}

async function saveModeratorInAcontest(ctab, fullCurl, moderators) {
  //we get tab, full contest url,moderator
  await ctab.bringToFront(); //bring tab to front
  await ctab.goto(fullCurl); //open full contest url in the tab
  await ctab.waitFor(3000); //wait for some time
